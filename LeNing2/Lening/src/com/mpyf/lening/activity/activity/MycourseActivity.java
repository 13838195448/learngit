...
public class MycourseActivity extends FragmentActivity {

	private LinearLayout ll_mycourse_back;
	private RelativeLayout rl_mycourse_zixiu,rl_mycourse_banji,rl_mycourse_gangwei;
	private TextView tv_mycourse_zixiu,tv_mycourse_banji,tv_mycourse_gangwei;
	...
	private void showdata(){
		...
		list.add(new Fragment_mycourse(1));
		list.add(new Fragment_mycourse(3));
		list.add(new Fragment_mycourse(2));
		
		Vpadapter adapter=new Vpadapter(getSupportFragmentManager(), list);
		vp_mycourse_top.setAdapter(adapter);
		
		vp_mycourse_top.setCurrentItem(getIntent().getIntExtra("item", 0));
		clearall(getIntent().getIntExtra("item", 0));
	}
	...
	private void addlistener(){
		ListenerServer.setfinish(MycourseActivity.this, ll_mycourse_back);
